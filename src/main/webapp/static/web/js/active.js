...
/*====== slick slider ======*/
	$('.center').slick({
	  ...
	  function showFormComment(){
	    var showCommment = $('#showCommment');
	    var form = $('.checkout_login');
	    showCommment.on('click' , function(e){
	        e.preventDefault();
	        
	        if ($(this).text() == "Gửi đánh giá của bạn") { 
		         $(this).text("Đóng lại"); 
		         $(this).parent().removeClass("cmt__btn");
		         $(this).parent().addClass("close__btn");
		     } else { 
		         $(this).text("Gửi đánh giá của bạn");
		         $(this).parent().removeClass("close__btn");
		         $(this).parent().addClass("cmt__btn");
		     }; 
	...
	$( "#showModel_btn" ).click(function() {
	    openModel();
	});

	$( ".close" ).click(function() {
	  //var model = $( ".close" ).closest( ".model" );
	  var model = $( ".close" ).closest( ".model-header" );
	  closeModel(model);
	});
	$( "#overplay" ).click(function() {
	 // const models = document.querySelectorAll('.model.active');
	  const models = document.querySelectorAll('.simple-model .active');
	  models.forEach(()=>{
	    closeModel();
	  });
	});
	...
	function closeModel() {
	  ...
	  $('.modelMsg').removeClass("active");
	  $('#overplay').removeClass("active");
	}
	
	$(".comment__form").validate({
		ignore: "",
		rules: {
			comment: {
				required: true,
		...
		messages: {
			comment: {
				required: "Yeu cau nhap binh luan",
		        minlength: "Toi thieu 10 ky tu",
		        maxlenghth : "Toi da 50 ky tu"
			},
			rating : {
				required: "Chon danh gia cua ban",
		        range: "Thap nhat la 1",
			}
		}
	});
	...
	$("#customer_address").validate({
		...
		messages: {
			fullName: "Yêu cầu điền tên",
			phone: {
				required: "Yêu cầu điền số điện thoại",
		        minlength: "Toi thieu 10 ky tu"
			},
			addr: "Yêu cầu nhập địa chỉ",
			provinceId: "Yêu cầu chọn thành phố",
			districtId: "Yêu cầu chọn quận huyện",
		}
	});
